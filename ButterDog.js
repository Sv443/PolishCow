...
/** Settings for Butter Dog */
const settings = {
    /** Set to true to enable debug messages */
    dbg: false,
    /** Path to the sound file */
    soundFile: resolve("./resources/butterdog.mp3"),
    ...
    animationDirectory: resolve("./resources/frames/"),
...
function startAnimation()
{
    ...
    const displayFrame = idx => {
        ...
        console.clear();
        if(settings.dbg)
            console.log(`Displaying frame #${idx}\n`);
    ...
    setInterval(() => {
        if(!animationPaused)
        {
            ...
            if(displayFrame(currentAnimSequenceIdx))
                currentAnimSequenceIdx++;

            if(currentAnimSequenceIdx >= frameSets[Object.keys(frameSets)[0]].length)
...
function getAllAnimationFrames()
{
    ...
    else
    {
        ...
        const sortFn = (a, b) => {
            ...
            let aNbr = parseInt(a.split(".")[0]);
            let bNbr = parseInt(b.split(".")[0]);
        ...
        fs.readdirSync(settings.animationDirectory).sort(sortFn).forEach(file => {
...
{
    return setWindowTitle(`Butter Dog - ${packageJson.author.name} [${process.stdout.columns}x${process.stdout.rows}]`);
}
